            ...
            <div className="w-full max-w-3xl mx-auto h-full bg-surface-elevated flex flex-col relative shadow-2xl border-x border-border/50">
                ...
                <div className="flex-1 overflow-y-auto pb-8 custom-scrollbar">
                    ...
                    <div className="py-10 flex justify-center bg-transparent">
                        <div className="relative w-[200px] h-[200px] rounded-full overflow-hidden cursor-pointer group shadow-2xl ring-4 ring-surface" onClick={() => fileInputRef.current.click()}>
                            ...
                            <div className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center text-white text-xs font-medium opacity-0 group-hover:opacity-100 transition-all duration-300 backdrop-blur-sm">
                    ...
                    {/* Name Section */}
                    <div className="bg-surface px-8 py-5 shadow-sm mb-3 border-y border-border/30">
                        <label className="text-primary text-sm font-medium mb-3 block">Your name</label>
                            ...
                            {isEditingName ? (
                                <div className="w-full border-b-2 border-primary pb-1 flex items-center">
                                    <input
                                        ...
                                        autoFocus
                                        maxLength={25}
                                        className="w-full text-text-1 text-[17px] bg-transparent outline-none placeholder:text-text-2/50"
                                    />
                                    <span className="text-xs text-text-2 mr-3">{25 - name.length}</span>
                                ...
                                <div className="w-full flex justify-between items-center group cursor-pointer" onClick={() => setIsEditingName(true)}>
                                    <span className="text-text-1 text-[17px]">{name}</span>
                        ...
                        {!isEditingName && <p className="text-[13px] text-text-2 mt-3 leading-tight">This is not your username or pin. This name will be visible to your WhatsApp contacts.</p>}
                    </div>

                    {/* About Section */}
                    <div className="bg-surface px-8 py-5 shadow-sm mb-3 border-y border-border/30">
                        <label className="text-primary text-sm font-medium mb-3 block">About</label>
                        <div className="flex items-center justify-between min-h-[30px]">
                            {isEditingAbout ? (
                                <div className="w-full border-b-2 border-primary pb-1 flex items-center">
                                    <input
                                        ...
                                        className="w-full text-text-1 text-[17px] bg-transparent outline-none placeholder:text-text-2/50"
                                    />
                                    <span className="text-xs text-text-2 mr-3">{139 - about.length}</span>
                                ...
                                <div className="w-full flex justify-between items-center group cursor-pointer" onClick={() => setIsEditingAbout(true)}>
                                    <span className="text-text-1 text-[17px] min-h-[1.5em]">{about}</span>
                                    <button className="text-primary opacity-0 group-hover:opacity-100 transition-all">
                                        <FaPen />
                                    </button>
                                </div>
                    ...
                    {/* Privacy Section */}
                    <div className="bg-surface px-8 py-5 shadow-sm mb-3 mt-8 border-y border-border/30">
                        <div className="text-primary text-sm font-medium mb-5">Privacy</div>

                        <div className="flex justify-between items-center mb-6">
                            <div className="flex flex-col gap-1">
                                <span className="text-text-1 text-[16px]">Share Location</span>
                                <span className="text-text-2 text-[13px]">Share your login location for security</span>
                            </div>
                            <label className="relative inline-block w-[40px] h-[24px]">
                                ...
                                />
                                <span className="absolute cursor-pointer top-0 left-0 right-0 bottom-0 bg-gray-300 dark:bg-gray-600 transition-all duration-300 rounded-full peer-checked:bg-primary before:absolute before:content-[''] before:h-[18px] before:w-[18px] before:left-[3px] before:bottom-[3px] before:bg-white before:transition-all before:duration-300 before:rounded-full peer-checked:before:translate-x-[16px] shadow-inner"></span>
                            </label>
                        </div>

                        <div className="flex justify-between items-center mb-6">
                            <div className="flex flex-col gap-1">
                                <span className="text-text-1 text-[16px]">Last Seen</span>
                                <span className="text-text-2 text-[13px]">Who can see when you're online</span>
                            </div>
                            <select
                                value={lastSeenPrivacy}
                                onChange={(e) => updatePrivacy('lastSeen', e.target.value)}
                                className="p-2 border border-border rounded-lg text-sm text-text-1 bg-surface-elevated outline-none focus:border-primary transition-colors cursor-pointer"
                            >
                        ...
                        <div className="flex justify-between items-center">
                            <div className="flex flex-col gap-1">
                                <span className="text-text-1 text-[16px]">Read Receipts</span>
                                <span className="text-text-2 text-[13px]">If turned off, you won't send receipts.</span>
                            </div>
                            <label className="relative inline-block w-[40px] h-[24px]">
                                <input
                                    type="checkbox"
